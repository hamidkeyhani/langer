...
actual class PlatformConvexVerifier actual constructor(convexUrl: String) {
    private val mockState = MutableStateFlow(TestAppState("mock-id", "Mock Status (iOS - Simulated)"))
    ...
    actual suspend fun updateTaskState(
        ...
        mockState.value = TestAppState(
            taskId = taskId,
            taskStatus = "$status (iOS Simulated)",
    ...
    actual suspend fun triggerGeneration(url: String, taskId: String) {
        mockState.value = TestAppState(
            taskId = taskId,
            taskStatus = "AI Generation Triggered (iOS Simulated)",
    ...
    actual suspend fun getGeneratedDeck(deckId: String): TestDeck? {
        return TestDeck(
            id = deckId,
            name = "Vocab: iOS Web Preview",
            description = "Simulated iOS vocabulary deck",
            category = "Brainstorm"
        )
    }
